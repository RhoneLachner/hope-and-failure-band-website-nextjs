...
export const securityConfig: SecurityConfig = {
    ...
    rateLimit: {
        maxRequests: Number(process.env.NEXT_PUBLIC_RATE_LIMIT_REQUESTS) || 100,
        windowMs: Number(process.env.NEXT_PUBLIC_RATE_LIMIT_DURATION) || 3600,
    },
    csp: {
        enabled: process.env.NEXT_PUBLIC_CSP_ENABLED === 'true',
        directives: {
            'default-src': ["'self'"],
